...
#+end_src
** Amazon
Install =EmacsAmazonLibs= for work in Amazon. For more details about it, check internal [[https://w.amazon.com/bin/view/Emacs/][Emacs Wiki]].
#+begin_src emacs-lisp
  (straight-use-package
...
For enhanced Java support in eglot, use =eglot-java=. Install =lombok.jar= to support project that uses lombok.
#+begin_src emacs-lisp
...
Use =auto-dark-mode= to switch between light/dark mode according to the system variant.
#+begin_src emacs-lisp
